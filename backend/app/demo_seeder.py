...
def seed_demo_data() -> None:
    ...
    # Skip seeding if data already exists — check _memory directly to avoid
    # triggering FirestoreBackedStore._sync_from_remote() which can block on
    # network timeouts when Firestore is not configured.
    if job_service._jobs_db._memory or candidate_service._candidates_db._memory:
        return
    ...
    # Seed challenge submission ranking in background — get_candidate() seeks through
    # a 100k gzip file per row and must NOT block the startup event loop.
    import threading

    def _bg_challenge_seed_and_embed():
        import logging
        _log = logging.getLogger("hiresense_api")

        # 1. Seed challenge submission CSV ranking (if available)
        try:
            ...
            _log.error(f"Background challenge ranking seed failed: {e}")

        # 2. Refresh embeddings and rebuild FAISS indexes
        try:
            ...
            _log.error(f"Background embedding refresh failed: {e}")

    t = threading.Thread(target=_bg_challenge_seed_and_embed, daemon=True, name="startup-bg")
    t.start()
